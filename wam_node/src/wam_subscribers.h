...
class WamSubscribers : public rclcpp::Node
{
  ...
  //Message Times
    ...
    barrett::math::Vector<3>::type rt_angular_velocity_cmd_;
  ...
  //Libbarrett Systems
    ...
    Multiplier<double, barrett::math::Vector<3>::type, barrett::math::Vector<3>::type> angular_velocity_mult_;
  ...
  //Eigen::Quaterniond rt_cartesian_orientation_rl_;
    ...
    barrett::systems::Summer<barrett::math::Vector<3>::type> cart_orientation_summer_;
    barrett::systems::ExposedOutput<cp_type> linear_vel_direction_track_, current_cp_track_, cp_cmd_track_;
    barrett::systems::ExposedOutput<barrett::math::Vector<3>::type> angular_vel_direction_track_, current_rpy_track_;
  ...
  // If velocity control is activated, update values.
    if (!rt_linear_angular_velocity_status_)
    {
      ...
      // start with angular velocity at 0
        barrett::math::Vector<3>::type start_rpy;
        start_rpy(0) = 0;
        start_rpy(1) = 0;
        start_rpy(2) = 0;
...
WamSubscribers<DOF>::rtAngularVelocityCb(const ptr<rt_angular_vel_t> msg)
{
  //Initialize systems
    if (!rt_angular_velocity_status_)
    {
        ...
        angular_vel_direction_track_.setValue(
...
WamSubscribers<DOF>::updateRT()
{
  // Checks if messages have been published within timeout
    if (time_t(rt_joint_position_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_linear_angular_velocity_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_angular_velocity_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_linear_velocity_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_cart_position_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_cart_pose_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_cart_orientation_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
    else if (time_t(rt_joint_velocity_msg_time_.nanoseconds() + kRtMsgTimeout.nanoseconds())  > rclcpp::Clock().now())
    {
    }
