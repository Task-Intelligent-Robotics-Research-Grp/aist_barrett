...
class BhandPublishers : public rclcpp::Node
{
  ...
  public:
    explicit BhandPublishers(Hand &hand, bool found_wam)
        : Node("BhandPublishers"), hand_(hand)
    {
        ...
        {
            tactile_state_pub_
                = this->create_publisher<bhand_msgs::msg::TactileStateArray>(
                    "/bhand/TactileStates", 100);
            RCLCPP_INFO(this->get_logger(), "Found Tactile Sensors");
        }
        if (found_ft_torque_)
        {
            finger_tip_torque_pub_
                = this->create_publisher<bhand_msgs::msg::FingerTipTorques>(
                    "/bhand/FingertipTorques", 100);
            RCLCPP_INFO(this->get_logger(), "Found FingerTip Torque Sensors");
        }

      //Publishes sensors states every 20ms
        sensor_pub_timer_
            = this->create_wall_timer(20ms,
          ...
          //WAM Publishers handles bhand joint states. Only publish these if no WAM
            ...
            bhand_joint_position_pub_timer_
                = this->create_wall_timer(
                    2ms, std::bind(&BhandPublishers::publishJointPositions,
                                   this));
        }
    }
